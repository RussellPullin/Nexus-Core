 ...
 */
router.post('/masters/:id/render', async (req, res) => {
  try {
    const orgId = requesterOrgId(req);
    if (!orgId) return res.status(404).json({ error: 'No organisation for this user' });
    ...
    await sendRender(res, req.query?.preview === '1' ? 'inline' : 'attachment', {
      masterId: req.params.id,
      orgId,
      participant,
      staff,
...
/**
 * GET preview — branded for the logged-in organisation. Optional `?participant_id=` /
 * `?staff_id=` fill person-specific fields; otherwise those stay blank.
 */
...
async function sendRender(res, disposition, opts) {
  const result = await renderLibraryDocument(opts);
